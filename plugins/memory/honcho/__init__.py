...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        try:
            ...
            # Gateway sessions should bind memory to the originating user instead
            # of a static shared peer_name.
            _gw_user_id = kwargs.get("user_id")
            # Preserve an explicitly configured peer_name on the validated
            # Honcho config object, but keep gateway user_id scoping as the
            # safe default for legacy/test doubles and unset configs.
            if _gw_user_id and (not cfg.peer_name or not isinstance(cfg, HonchoClientConfig)):
