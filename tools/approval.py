...
def check_all_command_guards(command: str, env_type: str,
    ...
    try:
        from tools.tirith_security import (
            ...
            ensure_installed as _ensure_tirith_installed,
            _load_security_config as _load_tirith_security_config,
        )

        tirith_cfg = _load_tirith_security_config()
        if (
            (is_gateway or is_ask)
            and tirith_cfg.get("tirith_enabled", True)
            and str(tirith_cfg.get("tirith_path", "tirith")).strip() == "tirith"
        ):
            resolved_tirith = _ensure_tirith_installed(log_failures=False)
            if resolved_tirith is None:
                if tirith_cfg.get("tirith_fail_open", True):
                    tirith_result = {
                        "action": "allow",
                        "findings": [],
                    ...
                    tirith_result = {
                        "action": "block",
                        "findings": [],
                        "summary": "tirith unavailable while background install is in progress",
                    }
            else:
