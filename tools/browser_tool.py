...
_SANE_PATH = os.pathsep.join(_SANE_PATH_ENTRIES)


def _desktop_node_runtime_dirs(node_home: str) -> list[str]:
    """Return PATH entries for a bundled desktop Node runtime."""
    if not node_home:
        return []
...
def _find_agent_browser() -> str:
    ...
    bundled_node_home = os.getenv("HERMES_DESKTOP_NODE_HOME", "").strip()
    if bundled_node_home:
        extra_dirs.extend(_desktop_node_runtime_dirs(bundled_node_home))

    hermes_home = get_hermes_home()
    extra_dirs.extend(_desktop_node_runtime_dirs(str(hermes_home / "node")))
...
def _run_browser_command(
    ...
    try:
        ...
        desktop_node_dirs = _desktop_node_runtime_dirs(os.getenv("HERMES_DESKTOP_NODE_HOME", "").strip())
        hermes_home = get_hermes_home()
        candidate_dirs = (
            desktop_node_dirs
            + _desktop_node_runtime_dirs(str(hermes_home / "node"))
