...
def supports_systemd_services() -> bool:
    if not is_linux() or is_termux() or is_container():
        return False
    # Require a local ``systemctl`` binary on real Linux hosts.  In unit tests
    # we also honor an explicitly monkeypatched ``shutil.which`` probe, but we
    # avoid consulting the non-Linux host environment when tests simulate Linux.
    if sys.platform.startswith("linux") or getattr(shutil.which, "__module__", "shutil") != "shutil":
