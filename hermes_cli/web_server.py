...
def _merge_desktop_session_metadata(session: Dict[str, Any]) -> Dict[str, Any]:
    """Best-effort merge of desktop-only metadata into SessionDB rows.

    Current SessionDB does not have a first-class cwd column, so the desktop
    sidecar persists per-session cwd in a tiny JSON store and mirrors it in
    ``model_config`` for new sessions.  This helper enriches existing API
    responses without requiring a DB migration.
    """
    try:
        from hermes_server.desktop_state import get_desktop_state_store
        meta = get_desktop_state_store().get_session_meta(str(session.get("id") or ""))
...
async def get_sessions():
    try:
        ...
        try:
            ...
            now = time.time()
            for s in sessions:
                _merge_desktop_session_metadata(s)
                s["is_active"] = (
...
async def get_session_detail(session_id: str):
    ...
    try:
        ...
        return _merge_desktop_session_metadata(session)
    finally:
        db.close()
