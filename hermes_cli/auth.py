...
def _resolve_minimax_base_url(default_url: str, env_override: str) -> str:
    """Normalize MiniMax base URLs, upgrading the legacy ``/v1`` path."""
    if not env_override:
        return default_url

    normalized = env_override.rstrip("/")
    if normalized.endswith("/v1"):
        return normalized[:-3] + "/anthropic"
    return normalized
...
def get_api_key_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id == "kimi-coding":
        ...
        base_url = _resolve_minimax_base_url(pconfig.inference_base_url, env_url)
    elif env_url:
        base_url = env_url
    else:
