...
def _is_home_openclaw_dir(path: Path) -> bool:
    """Return True when *path* is one of the default OpenClaw home dirs."""
    try:
    ...
    for name in _OPENCLAW_DIR_NAMES:
        try:
            if resolved == (Path.home() / name).resolve():
                return True
        except Exception:
            continue
    return False
...
def _cmd_cleanup(args):
    ...
    running = _detect_openclaw_processes() if any(_is_home_openclaw_dir(d) for d in dirs_to_check) else []
    if running:
        print()
        ...
        print()
        if dry_run:
            print_info("Dry run only — showing archive preview without blocking on the running service.")
        elif not auto_yes:
            if not sys.stdin.isatty():
