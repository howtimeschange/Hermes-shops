    ...
    def test_finds_in_bundled_desktop_node_runtime(self):
        ...
        def mock_which(cmd, path=None):
            if cmd == "agent-browser" and path and bundled_bin in path.split(os.pathsep):
                return f"{bundled_bin}/agent-browser"
            return None
        ...
        with patch.dict(os.environ, {"HERMES_DESKTOP_NODE_HOME": bundled_home}, clear=False), \
            ...
            result = _find_agent_browser()
            assert result == f"{bundled_bin}/agent-browser"
...
class TestRunBrowserCommandPathConstruction:
    ...
    def test_subprocess_path_includes_bundled_desktop_runtime(self, tmp_path):
        """Bundled desktop node runtime directories should be prepended for subprocesses."""
        captured_env = {}
        ...
        fake_json = json.dumps({"success": True})
        bundled_home = tmp_path / "node-runtime"
        bundled_bin = bundled_home / "bin"
        bundled_dot_bin = bundled_home / "node_modules" / ".bin"
        bundled_bin.mkdir(parents=True)
        bundled_dot_bin.mkdir(parents=True)

        with patch("tools.browser_tool._find_agent_browser", return_value=str(bundled_dot_bin / "agent-browser")), \
             patch("tools.browser_tool._get_session_info", return_value=fake_session), \
             patch("tools.browser_tool._socket_safe_tmpdir", return_value=str(tmp_path)), \
             patch("tools.browser_tool._discover_homebrew_node_dirs", return_value=[]), \
             patch("subprocess.Popen", side_effect=capture_popen), \
             ...
             patch.dict(
                 os.environ,
                 {
                     "PATH": "/usr/bin:/bin",
                     "HOME": "/home/test",
                     "HERMES_DESKTOP_NODE_HOME": str(bundled_home),
                 },
                 clear=True,
             ):
            with patch("builtins.open", mock_open(read_data=fake_json)):
        ...
        result_path = captured_env.get("PATH", "")
        assert str(bundled_bin) in result_path
        assert str(bundled_dot_bin) in result_path
