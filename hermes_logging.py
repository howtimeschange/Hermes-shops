...
def setup_logging(
    ...
    *force* is ``True``.
    ...
    # Reset Hermes-owned loggers to propagate through root file handlers.
    # Tests and long-lived worker processes can leave component loggers with
    # sticky WARNING levels, which makes agent.log look incomplete even though
    # it is intended to be the catch-all sink.
    managed_prefixes = tuple(prefix for prefixes in COMPONENT_PREFIXES.values() for prefix in prefixes)
    managed_names = set(managed_prefixes)
    ...
    for name in managed_names:
        ...
        if component_logger.level > level:
