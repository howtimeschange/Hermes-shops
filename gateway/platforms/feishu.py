...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _build_event_handler(self) -> Any:
        ...
        # Newer SDKs expose chat-member lifecycle hooks, but older test doubles
        # and older SDK versions may not. Register them opportunistically.
        register_added = getattr(builder, "register_p2_im_chat_member_bot_added_v1", None)
        if callable(register_added):
            builder = register_added(self._on_bot_added_to_chat)
        register_deleted = getattr(builder, "register_p2_im_chat_member_bot_deleted_v1", None)
        if callable(register_deleted):
            builder = register_deleted(self._on_bot_removed_from_chat)
