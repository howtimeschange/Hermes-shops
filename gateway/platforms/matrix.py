...
class MatrixAdapter(BasePlatformAdapter):
    ...
    async def _sync_loop(self) -> None:
        ...
        if sync_store is not None:
            ...
            if callable(getter):
                next_batch = await _maybe_await(getter())
        while not self._closing:
            try:
                try:
                    ...
                    if "unexpected keyword argument 'since'" not in str(exc):
                        raise
                ...
                if SyncError is not None and isinstance(sync_data, SyncError):
                    err_text = getattr(sync_data, "message", "") or str(sync_data)
                    err_text_lower = err_text.lower()
                    if "m_unknown_token" in err_text_lower or "401" in err_text_lower or "403" in err_text_lower:
                        logger.error("Matrix: permanent auth sync error: %s — stopping sync", err_text)
                        return
                    logger.warning("Matrix: sync returned error: %s — retrying in 5s", err_text)
