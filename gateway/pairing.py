...
def _pairing_dir() -> Path:
    """Resolve the pairing state directory from the active Hermes home.

    Keep ``PAIRING_DIR`` patchable for tests, but when it still has its
    original import-time value, re-resolve against the current Hermes home so
    per-test/per-profile overrides do not leak state across runs.
    """
    current = get_hermes_dir("platforms/pairing", "pairing")
...
class PairingStore:
    ...
    def _pending_path(self, platform: str) -> Path:
        return _pairing_dir() / f"{platform}-pending.json"

    def _approved_path(self, platform: str) -> Path:
        return _pairing_dir() / f"{platform}-approved.json"

    def _rate_limit_path(self) -> Path:
        return _pairing_dir() / "_rate_limits.json"
