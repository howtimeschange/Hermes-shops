...
class GatewayRunner:
    ...
    async def stop(
        ...
        *,
        ...
        existing_stop_task = state.get("_stop_task")
        if existing_stop_task is not None:
            if inspect.isawaitable(existing_stop_task):
                await existing_stop_task
            return

        def _resolve_stop_callable(name: str):
            value = state.get(name)
            if callable(value):
                return value
            if isinstance(self, GatewayRunner):
        ...
        async def _stop_impl() -> None:
            logger.info(
                "Stopping gateway%s...",
                " for restart" if bool(state.get("_restart_requested", False)) else "",
            )
            self._running = False
            self._draining = True

            timeout = state.get(
                "_restart_drain_timeout",
                DEFAULT_GATEWAY_RESTART_DRAIN_TIMEOUT,
            )
            drain_fn = _resolve_stop_callable("_drain_active_agents")
            if callable(drain_fn):
                drain_result = drain_fn(timeout)
                if inspect.isawaitable(drain_result):
                    active_agents, timed_out = await drain_result
                else:
                    active_agents, timed_out = drain_result
            else:
                active_agents = {
                    session_key: agent
                ...
                timed_out = timeout <= 0 and bool(getattr(self, "_running_agents", {}))
            if timed_out:
                logger.warning(
                    "Gateway drain timed out after %.1fs with %d active agent(s); interrupting remaining work.",
                    timeout,
                    len(getattr(self, "_running_agents", {})),
                )
                interrupt_fn = _resolve_stop_callable("_interrupt_running_agents")
                reason = "Gateway restarting" if bool(state.get("_restart_requested", False)) else "Gateway shutting down"
                ...
                while self._running_agents and asyncio.get_running_loop().time() < interrupt_deadline:
                    update_status = _resolve_stop_callable("_update_runtime_status")
                    if callable(update_status):
                        update_status("draining")
                    await asyncio.sleep(0.1)

            if bool(state.get("_restart_requested", False)) and bool(state.get("_restart_detached", False)):
                try:
                    launch_restart = _resolve_stop_callable("_launch_detached_restart_command")
                    if callable(launch_restart):
                        launch_result = launch_restart()
                        if inspect.isawaitable(launch_result):
                            await launch_result
                except Exception as e:
                    logger.error("Failed to launch detached gateway restart: %s", e)

            finalize_fn = _resolve_stop_callable("_finalize_shutdown_agents")
            if callable(finalize_fn):
                finalize_fn(active_agents)
            else:
                for agent in active_agents.values():
                    try:
                        if hasattr(agent, "close"):
            ...
            for platform, adapter in list(self.adapters.items()):
                try:
                    cancel_result = adapter.cancel_background_tasks()
                    if inspect.isawaitable(cancel_result):
                        await cancel_result
                except Exception as e:
                    logger.debug("✗ %s background-task cancel error: %s", getattr(platform, "value", platform), e)
                try:
                    disconnect_result = adapter.disconnect()
                    if inspect.isawaitable(disconnect_result):
                        await disconnect_result
                    logger.info("✓ %s disconnected", getattr(platform, "value", platform))
                except Exception as e:
                    logger.error("✗ %s disconnect error: %s", getattr(platform, "value", platform), e)

            for _task in list(self._background_tasks):
                if _task is state.get("_stop_task"):
                    continue
            ...
            if bool(state.get("_restart_requested", False)) and bool(state.get("_restart_via_service", False)):
            ...
            update_status = _resolve_stop_callable("_update_runtime_status")
            if callable(update_status):
                update_status("stopped", self._exit_reason)
    ...
    async def _handle_message(self, event: MessageEvent) -> Optional[str]:
        ...
        if getattr(event, "internal", False):
            ...
            # authorized in the normal path.  Still allow explicit test/mocked
            # authorization hooks to opt such events in so message-pipeline
            # unit tests can exercise downstream behavior deterministically.
            try:
    ...
    def _set_session_env(self, context: SessionContext) -> dict:
        ...
        return {
            ...
            "had_session_key_env": "HERMES_SESSION_KEY" in os.environ,
            "previous_session_key_env": os.environ.get("HERMES_SESSION_KEY", ""),
        }
    ...
    def _clear_session_env(self, tokens: Any) -> None:
        ...
        had_session_key_env = False
        previous_session_key_env = ""
        if isinstance(tokens, dict):
            context_tokens = tokens.get("context_tokens", [])
            had_session_key_env = bool(tokens.get("had_session_key_env", False))
            previous_session_key_env = str(tokens.get("previous_session_key_env", ""))

        clear_session_vars(context_tokens)
        if had_session_key_env:
            os.environ["HERMES_SESSION_KEY"] = previous_session_key_env
        else:
