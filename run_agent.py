            ...
            try:
                ...
                if assistant_message.tool_calls:
                    ...
                    if not self._has_content_after_think_block(final_response):
                        ...
                        _reasoning_text = _structured_reasoning
                        if not _reasoning_text:
                            _inline_think_blocks = re.findall(
                                r"<think>(.*?)</think>",
                                final_response or "",
                                flags=re.DOTALL,
                            )
                            if _inline_think_blocks:
                                _reasoning_text = "\n\n".join(
                                    block.strip()
                                    for block in _inline_think_blocks
                                    if block and block.strip()
                                ) or None
                        _has_structured = bool(_reasoning_text)
                        if _structured_reasoning and self._thinking_prefill_retries < 2:
